...
### 動態 context · 相關 KPI (RAG)
{{ rag_kpi }}
{%- endif %}
{%- if rag_few_shot %}

### 動態 context · 過往成功案例 (RAG)
{{ rag_few_shot }}
{%- endif %}
...
def compose_phase_b_prompt_modular(intent: str, cols_info: str,
                                     ...
                                     dashboard_block: str = "",
                                     rag_anti_pattern: str = "",
                                     rag_few_shot: str = "") -> str:
    """
    v0.6.0:依 preprocess intent 組裝 slim Phase B prompt。
    v0.16.0+(M6.3):加 RAG slot kwargs(預設空 → byte-equal v0.15)。
    ...
    Args:
        ...
        rag_anti_pattern: RAG anti-pattern slot(空 → 無 RAG 區塊)
        rag_few_shot: RAG few-shot slot(空 → 無 RAG 區塊)

    Returns:
        組裝後的完整 prompt(string)。

    設計細節:
        - HEADER 走 Jinja2 render(cols_info / domain_knowledge / rag_* 是變數注入)
    ...
    header = Template(_PHASE_B_HEADER_TEMPLATE_V6).render(
        ...
        rag_anti_pattern=rag_anti_pattern,
...
def compose_phase_c_prompt_modular(intent: str, cols_info: str,
                                     echarts_few_shot: str = "",
                                     rag_chart_recipe: str = "",
                                     rag_anti_pattern: str = "") -> str:
    """
    ...
    Args:
        ...
        rag_chart_recipe: RAG chart-recipe slot(空 → 無 RAG 區塊)
        rag_anti_pattern: RAG anti-pattern slot(空 → 無 RAG 區塊)
    ...
    """
    from jinja2 import Template
    header = Template(_PHASE_C_HEADER_TEMPLATE).render(
        cols_info=cols_info,
        rag_chart_recipe=rag_chart_recipe,
        rag_anti_pattern=rag_anti_pattern,
    )
