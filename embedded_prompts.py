...
0. 🚨【欄位名鎖死 + Q 是 post-aggregation 終態】(CRITICAL FATAL — v0.8.8 強化)
   ...
   - **Aggregated wide**(本 case 占多數):多個欄位有 KPI 名後綴
     (`_count` / `_rate` / `_sum` / `_avg` / `_pct`):
     `[company_code, pay_count, return_count, ai_rate]`
     → 多 series 用「**每個 KPI column 一個 series**」(NOT filter)。
   - **Long / tidy**:有明顯 dim + sub_dim + value 3 欄結構(sub_dim 是
     raw enum 字串):
     `[company_code, category, count]` 或 `[company_code, mechanism, count]`
     → 多 series 用 filter sub_dim 值(見 rule 3.2)。

   ⚠️ 沒看到 long-format 樞紐欄位(`category` / `mechanism` 等)就是
   **wide aggregated**,直接用 KPI 欄位,**完全禁止 filter**。
