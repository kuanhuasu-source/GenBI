...
class UploadService:
    ...
    def regenerate_metadata(
        ...
        for table in tables:
            t_id = table["table_id"]
            t_profile = profile_lookup.get(t_id)
            if not t_profile:
                skipped_tables.append(t_id)
                logger.warning(
                    f"regenerate_metadata: no profile for table_id=`{t_id}` "
                    f"in dataset `{dataset_id}`, skipping this table"
                )
                continue
            column_profiles = t_profile.get("columns", [])
            ...
            per_table_md = upload_metadata_generator.build_metadata(
                dataset_doc=dataset,
                table_doc=table,
        ...
        if not merged_collections:
            raise ValueError(
                f"Dataset `{dataset_id}` 沒有任何 table 有可用 profile — "
                f"skipped: {skipped_tables}"
            )

        # Final metadata = dataset-level fields from last per-table md +
        # all collections merged. (last_per_table_md is non-None when
        # merged_collections is non-empty.)
        ...
        if len(merged_collections) > 1:
            total_rows = sum(
                t.get("row_count", 0) for t in tables
                if t["table_id"] in merged_collections
            )
            ds_name = (
                dataset.get("dataset_name") or dataset.get("_id") or "Dataset"
            )
            biz_ctx = dict(metadata.get("business_context") or {})
            biz_ctx["business_description"] = (
                f"User uploaded multi-table dataset `{ds_name}` for ad-hoc "
                f"analysis. {len(merged_collections)} tables, "
                f"{total_rows:,} total rows."
            )
            metadata["business_context"] = biz_ctx
        ...
        version = self.repo.save_metadata_version(
            ...
            notes=(
                ...
                f"{len(merged_collections)} table(s) · "
                f"LLM-assisted={use_llm}"
                + (f" · skipped {skipped_tables}" if skipped_tables else "")
            ),
            activate=True,
        )
        logger.info(
