...
### 🌟 META 結構性問題 — 用 `[META]` 標記短路,**完全不走 Phase A/B/C/D**
若使用者問的是 dataset 的**結構**(非數據分析)— 例:
- 「列出有哪些 sheet / table」
- 「每張表 row count 與可能主鍵」
- 「欄位類型 / schema」
- 「dataset 描述」

**第一個字元必須是 `[META]`**(同 `[REFUSE]` 的協定),然後直接以 markdown
回答結構性問題。Domain Knowledge 已有完整答案(`# Collections & 欄位定義`
區段列出所有 sheet + row 數 + primary_key + 欄位類型 + grain)— 直接抄出來
列成表格或清單。
...
| 表名 | 列數 | 可能主鍵 | 描述 |
|---|---|---|---|
| <table_id> | <row_count> | <pk> | <grain> |
...
```

⚠️ 一旦你輸出 `[META]`,系統會 **跳過 Phase A/B/C/D**,直接把你寫的內容回給
使用者。所以答案要在這裡寫完整,不要寫「Phase B 會處理」之類的占位。

⚠️⚠️⚠️ **`[META]` 的使用界線(超重要,違反會誤導使用者)**:
**只有 100% 結構性問題**才能用 `[META]`。問題裡只要有任何「動作詞」就不行:

❌ **絕對不可用 `[META]`** 的問題模式(雖然句中可能也提到 sheet / 欄位):
- 「**統計** 各部門加班時數」 → 含「統計」是計算動作,走 A/B/C
- 「**計算** 訓練完成率」 → 含「計算」,走 A/B/C
- 「依 department_name **聚合**」 → 含「依...聚合」是 groupby,走 A/B/C
- 「**比較** 不同部門遲到率」 → 含「比較」,走 A/B/C
- 「**排名** TOP-5 員工」 → 含「排名」,走 A/B/C
- 「**畫** 一張長條圖」 / 「用**表格**呈現結果」 → 含視覺化動作,走 A/B/C
- 「**篩選** 2026/01-2026/02 的紀錄」 → 含「篩選」,走 A/B/C
- 「**平均 / 總和 / 加總 / 占比 / 趨勢**」任何一個都不可用 `[META]`

✅ **可以用 `[META]` 的問題模式**(純查詢 metadata,不需要碰實際數據):
- 「這份資料有哪些 **sheet** / table?」
- 「每張表的 **row count** 和 **可能主鍵**?」
- 「**欄位類型** / schema 是什麼?」
- 「**grain** 是什麼意思?」
- 「dataset 描述 / 用途?」

決策口訣:**若答案需要讀 raw DataFrame 任何一格 → 不可用 `[META]`**。
