...
5.55 ⚠️【Long format Q + Stacked Bar 對齊鐵律】(CRITICAL — 容易踩坑)
    若 Q 是 **long format**(每列代表一個 (dim_a, dim_b) 組合,如 company × category),
    要畫 stacked bar 時 **xAxis.data 必須用 `Q['dim_a'].unique().tolist()` 而非 `.tolist()`**!

    ❌ 反例(會炸):
    ```python
    "xAxis": {{"data": Q["company_code"].tolist()}}    # ← 重複 N 次 (TSC, TSC, TSC, TSN, ...)
    "series": [
        {{"data": Q[Q['category']=='X']['pct'].tolist()}}  # ← 3 個值跑去填前 3 個位置
    ]
    # → 所有 bar 擠在前幾個位置,後面空白
    ```
    ...
    ✅ 正解 (pivot 對齊):
    ```python
    companies = Q['company_code'].unique().tolist()
    categories = Q['application_category'].unique().tolist()
    pivot = Q.pivot_table(index='company_code', columns='application_category',
                          values='percentage', aggfunc='sum').reindex(companies).fillna(0)
    "xAxis": {{"data": companies}},
    "series": [
        {{"name": cat, "type": "bar", "stack": "pct",
          "data": (pivot[cat] * 100).round(2).tolist()}}
