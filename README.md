...
## 🖥️ Sidebar 兩組導航(v0.17+ · `st.navigation`)

啟動 `streamlit run app.py` 後 sidebar 自動分組顯示(分析工作區 expanded、Admin collapsed):

### 📊 分析工作區(end-user)

| Page | 功能 |
|---|---|
| **💬 Schema-driven 分析** (`pages/main_chat.py`, **default**)| 對話式 BI · 5-phase workflow + domain switcher(原 app.py,v0.17 拆出來)|
| **📊 Upload 分析** (`pages/08_data_analysis.py`, v0.17+)| Dataset picker + 漸進 phase 渲染 + Save Asset + Debug Panel |
| **📤 資料準備** (`pages/07_data_workspace.py`, v0.17+)| BYOD · 上傳 / Profile / Field Review / Status code / Grain / Confirm metadata |
| **⭐ 已存圖表** (`pages/09_saved_assets.py`, v0.13.2+)| Saved Chart / Metric / Template 瀏覽 + rerun / rename / 軟刪除 |

### ⚙️ 系統管理 Admin(ops / dev)

| Page | 功能 |
|---|---|
| **🗂️ Metadata 編輯** (`04_metadata.py`)| Per-domain metadata JSON 編輯 + 新增 domain 精靈 + 版本歷史 |
| **📝 Prompt 編輯** (`03_prompts.py`)| 5 phase × domain 的 Jinja2 模板編輯 + Render preview + 版本啟用 |
| **🧪 測試案例** (`01_test_cases.py`)| Test case CRUD + tag 篩選 + activate/deactivate |
| **📊 測試紀錄** (`02_test_runs.py`)| 歷史 runs(per domain)+ baseline mark + side-by-side compare |
| **🔍 Trace 追蹤** (`05_task_traces.py`, v0.7.0+)| 每次 query 完整 phase + LLM call 記錄 |
| **🤖 自學審核** (`06_learning_review.py`, v0.9.0+)| Self-learning admin:metric dashboard / candidate review / contradiction review / observation browser |
...
├── pages/
│   ├── main_chat.py               # v0.17+ · Schema-driven 主對話(從 app.py 拆出)
│   ├── 01_test_cases.py
│   ├── 02_test_runs.py
│   ├── 03_prompts.py
│   ├── 04_metadata.py
│   ├── 05_task_traces.py
│   ├── 06_learning_review.py
│   ├── 07_data_workspace.py       # v0.17+ · Upload + Profile + Metadata Review(原 07 sections 1-9)
│   ├── 08_data_analysis.py        # v0.17+ · Chat + Save Asset + Debug(原 07 sections 10-12)
│   └── 09_saved_assets.py         # 從 v0.13.2 saved_assets,v0.17 rename
├── migrations/
│   ├── 001_seed_prompts.py
...
├── requirements.txt               # v0.17+ · streamlit>=1.36(st.navigation stable)
├── .env.example
└── data/                          # tFlex 合成原始資料
```
...
- **v0.17.0** 🎨 **UI Refactor**(3 sprints,UX 三痛點一次解):
  - **A1 · Page split**:原 1608 行 `07_upload_workspace.py` 拆成 `07_data_workspace`(資料準備)+ `08_data_analysis`(分析)+ `09_saved_assets`(已存圖表)
