...
# ============================================================
# v0.18 fix: full-width punctuation sanitizer
# ============================================================
# Chinese-tuned LLMs occasionally emit Python code with full-width CJK
# punctuation (`，` instead of `,`, `（` instead of `(`, etc.) when the
# generation context is in Chinese. Python's parser only accepts ASCII
# punctuation — these chars produce:
#     SyntaxError: invalid character '，' (U+FF0C)
# which can't be recovered without rewriting the code.
#
# This sanitizer normalizes the 9 most common full-width chars to their
# ASCII equivalents before exec. Trade-off: if a string literal in the
# code legitimately contains CJK punctuation (e.g. comparing against
# data value "你好,世界"), this would also rewrite that — but BI code
# generated by LLMs almost never has CJK punctuation inside string
# literals (filter values come from column names + sample data, which
# the parser_phase normalize to ASCII anyway). Empirical: zero false
# positives observed in the M1-M7 test suite.
...
def normalize_code_punctuation(code: str) -> str:
    """Translate common full-width CJK punctuation to ASCII so Python's
    parser accepts the code. Idempotent."""
    if not code:
        return code
    return code.translate(_CJK_PUNCT_MAP)
    ...
    def _handle_query_inner(
        ...
        for attempt in range(3):
            try:
                ...
                # v0.18 fix: LLM 偶爾 emit 全形標點 → SyntaxError。先洗一輪。
                phase_a_code = normalize_code_punctuation(phase_a_code)
        ...
        for attempt in range(3):
            try:
                ...
                # v0.18 fix: 同 Phase A,洗掉全形標點。
                phase_b_code = normalize_code_punctuation(phase_b_code)
        ...
        for attempt in range(3):
            try:
                ...
                # v0.18 fix: 同 Phase A/B,洗掉全形標點。
                phase_c_code = normalize_code_punctuation(phase_c_code)
