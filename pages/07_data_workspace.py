...
st.markdown(
    "<h1 style='font-size:2.2rem;margin:0 0 .3rem 0'>📤 Data Workspace</h1>"
    "<p style='color:#8B6F4A;font-size:0.95rem;margin:0 0 1rem 0'>"
    "BYOD · 上傳 CSV / Excel,解析欄位、產生 profile、Review + Confirm metadata。"
    "Confirm 後到 <b>📊 Data Analysis</b> 提問分析。"
    "</p>",
...
if mongo_db is None:
    st.error(
        f"⚠️ **MongoDB 連線失敗** — Data Workspace 需要 DB 才能運作。\n\n"
...
with st.sidebar:
    ...
    st.caption("v0.17 · Upload + Profile + Metadata Review")
    st.markdown("---")
    st.caption(
        "💡 **流程提示**\n\n"
        "1. 上傳檔案 → 自動解析 + profile\n\n"
        "2. Review metadata(語意 / grain / status code / 限制)\n\n"
        "3. Apply edits → Confirm metadata\n\n"
        "4. 進 **📊 Data Analysis** 聊天分析"
    )
...
# ============================================================
# 🔟 · 開始分析(v0.17:導向 pages/08_data_analysis.py)
# ============================================================
st.markdown("### 🔟 開始分析")

if md_status == "confirmed":
    st.success(
        f"✅ Metadata v{md_version} 已 confirmed — 可以進入分析階段。"
    )
    if st.button(
        "📊 開始分析此資料集 →",
        type="primary",
        use_container_width=False,
        help="跳到 Data Analysis page,以此 dataset 作為 default。",
    ):
        st.session_state["analysis_dataset_id"] = selected_id
        st.switch_page("pages/08_data_analysis.py")
else:
    st.info(
        "ℹ️ Metadata 尚未 confirmed — 請先在上方 **9️⃣ Apply / Confirm** "
        "區段按 **✅ Confirm metadata**,才能進入分析階段。"
    )
