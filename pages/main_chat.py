"""
pages/main_chat.py — v0.17.0 · Schema-driven Chat (從 app.py 拆出)

UI 重構於 v0.17:此頁是原 app.py 的本體(schema-driven 對話分析),
被 st.navigation 包成「💬 Schema-driven 分析」進入點。
set_page_config 統一在 app.py 設定,本檔不再呼叫。

# 5-phase pipeline(保留原 app.py 行 755-1408 邏輯)
Phase 0 → Plan       (LLM 規劃)
...
# v0.17:此檔位於 pages/,需把 project root 加進 sys.path 才能 import 頂層模組
...
# v0.17:assets 在 project root,改用 _PROJECT_ROOT 而非 __file__.parent
_LOGO_PATH = (
    _PROJECT_ROOT / "assets" / "hr_chatchart_4_svg_assets" / _LOGO_FILE
)
...
# v0.17:_PROJECT_ROOT 上頭已定義,assets 在 project root 下
_FALLBACK_LOGO = _PROJECT_ROOT / "assets" / "genbi_logo.svg"

# v0.17:set_page_config 統一改在 app.py(st.navigation main entry)設定,
# 本檔不再呼叫,避免 "set_page_config can only be called once" 警告。
